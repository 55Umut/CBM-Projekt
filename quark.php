...
<body>
 ...
 <script>
    // Wenn entweder der Boss oder der Spieler besiegt ist, 10 Sekunden warten und dann $unset1 und $unset2

        // Wenn entweder der Boss oder der Spieler besiegt ist, 13 Sekunden warten und dann auf 'spiel.php' weiterleiten
        if (<?php echo ($_SESSION['boss_lebenspunkte'] <= 0 || $_SESSION['spieler_lebenspunkte'] <= 0) ? 'true' : 'false'; ?>) {
            setTimeout(function() {
                window.location.href = 'spiel.php';
            }, 13000);
...
$unset1 = unset($_SESSION['spieler_lebenspunkte']);
$unset2 = unset($_SESSION['boss_lebenspunkte']); 
...
<body>
    ...
    <div class="attack-buttons">
        <div class="attacks-left">
            <button type="submit" name="angriff" value="angriff1" class="attack-button" id="angriff1">
                <?php echo htmlspecialchars($charakter_angriff1) . " - Schaden: " . $charakter_schaden1; ?>
            </button>
            <button type="submit" name="angriff" value="angriff2" class="attack-button" id="angriff2">
    ...
    <script>
    function playSound(angriff) {
        let soundFile = '';
        switch (angriff) {
            case 'angriff1':
                soundFile = 'nixgemacht.mp3';
                break;
            case 'angriff2':
                soundFile = 'haha.mp3';
                break;
            case 'angriff3':
                soundFile = 'schnarch.mp3';
                break;
            default:
                break;
        }
...
// Bossdaten und Angriffswerte laden
if (!isset($_SESSION['boss_name'])) {
    include_once 'bosse.php';
    $randomBoss = Boss::getRandomBoss();
    $_SESSION['boss_name'] = $randomBoss->getName();
...
if (isset($_POST['angriff'])) {
    $spieler_angriff = $_POST['angriff'];
    $spieler_schaden = 0;
    $sound = ''; // Variable für den Sound

    switch ($spieler_angriff) {
        case 'angriff1':
            $spieler_schaden = $charakter_schaden1;
            $sound = 'sound_angriff1.mp3'; // Beispiel Sound
            break;
        case 'angriff2':
            $spieler_schaden = $charakter_schaden2;
            $sound = 'sound_angriff2.mp3'; // Beispiel Sound
            break;
        case 'angriff3':
            $spieler_schaden = $charakter_schaden3;
            $sound = 'sound_angriff3.mp3'; // Beispiel Sound
            break;
        case 'spezial':
            $spieler_schaden = $charakter_schaden_spezial;
            $sound = 'sound_spezial.mp3'; // Beispiel Sound
            break;
    }

    // Schaden des Spielers auf den Boss anwenden
    $_SESSION['boss_lebenspunkte'] -= $spieler_schaden;
    ...
    // Schaden des Bosses auf den Spieler anwenden
    $_SESSION['spieler_lebenspunkte'] -= $boss_schaden;
    ...
    if ($_SESSION['boss_lebenspunkte'] <= 0) {
        ...
        $level = $_SESSION['level'];
        $details = "Benutzer: $benutzername, Level: $level, Charakter: $charakter_name"; 
        // Punkte in die Datenbank einfügen
        $sql_update_punkte = 'INSERT INTO nutzer_log (nutzer_id, aktion, punkte, level, charakter_id, status, details) values (?, ?, ?, ?, ?, ?, ?)';
        $stmt_update_punkte = $conn->prepare($sql_update_punkte);
        $test123 = 'Boss besiegt';
        $test345 = 'Gewonnen';
    ...
    // Falls der Spieler keine Lebenspunkte mehr hat
    if ($_SESSION['spieler_lebenspunkte'] <= 0) {
        $_SESSION['spieler_lebenspunkte'] = 0;
    }

    // Hier kannst du den Sound über JavaScript einfügen
    echo "<script>playSound('$sound');</script>";
}
$stmt = $conn->prepare("SELECT punkte FROM nutzer_log WHERE nutzer_id = ? ORDER BY id DESC LIMIT 1");
$stmt->bind_param("i", $nutzer_id);
$stmt->execute();
