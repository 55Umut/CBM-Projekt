...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['boss_id']) && isset($_POST['image'])) {
        ...
        // Das Bild in der Datenbank für den ausgewählten Boss speichern
        $stmt = $conn->prepare("UPDATE bosse SET bild_url = ? WHERE id = ?");
        $stmt->bind_param("si", $filePath, $boss_id);
        $stmt->execute();

        // Überprüfung, ob das Bild erfolgreich eingefügt wurde
        ...
        echo json_encode(['success' => false, 'message' => 'Fehler: Kein Boss ausgewählt oder kein Bild hochgeladen.']);
    }
}

// Alle Bosse aus der Datenbank laden
$bossesQuery = "SELECT id, name FROM bosse";
$bossesResult = $conn->query($bossesQuery);
$bosses = [];
if ($bossesResult->num_rows > 0) {
    while($row = $bossesResult->fetch_assoc()) {
        $bosses[] = $row;
    }
}
?>
...
<head>
    ...
    <title>Bild für Boss hochladen</title>
    <style>
        body {
...
<body>
    <div class="container">
        <h2>Wählen Sie einen Boss und laden Sie ein Bild hoch</h2>

        <!-- Auswahlfeld für Bosse -->
        <form id="upload-form" method="POST" enctype="multipart/form-data">
            <label for="boss-select">Wählen Sie einen Boss:</label>
            <select name="boss_id" id="boss-select">
                <?php foreach ($bosses as $boss): ?>
                    <option value="<?= $boss['id']; ?>"><?= htmlspecialchars($boss['name']); ?></option>
                <?php endforeach; ?>
            </select>
            <br><br>
    ...
    <script>
        ...
        uploadForm.addEventListener('submit', function (event) {
            ...
            uploadImage(bossId, imageData);
        });

        // Funktion zum Hochladen des Bildes
        function uploadImage(bossId, imageData) {
